<template>
	<div>
		...
		<div class="tab-container" v-show="isShow == 1">
			<el-form label-width="80px" :inline="true" v-model="formInline" class="demo-form-inline">
				<div class="title">
					<el-row>
						<el-col>
							<el-button v-show="permission.indexOf('141') != -1" type="primary" @click="addPurseMan" style="margin-right: 20px;">
			...
			<div class="tTable" id="printMe">
				<el-table :data="data" stripe border style="width: 100%;" size="mini" show-summary :summary-method="getSummaries">
					...
					<el-table-column prop="status" label="单据状态" width="100" align="center">
						<template slot-scope="scope">
							<span v-if="scope.row.status == 0" type="text">未报溢</span>
							<span v-if="scope.row.status == 1" type="text">已报溢</span>
						</template>
					</el-table-column>
		...
		<div class="tab-container" v-show="isShow == 2">
			...
			<el-row class="model_title">
				<el-col :span="12" style="border-left: #13C2C2 3px solid; padding-left: 15px;">
					<span v-if="!disabled">新增</span>报溢单
				</el-col>
				<el-col :span="12" style="text-align: right;">
			...
			<div class="title">
				<el-row v-show="!disabled">
					<el-col :span="24">
						...
						<!--编辑时候的保存-->
						<el-button v-show="permission.indexOf('143') != -1" type="primary" @click="EditOutOrder" v-if="showSave == 2">
							<svg-icon icon-class="icon_save" style="margin-right: 10px;" /> <span>保存</span>
						</el-button>
						<!--查看的时候没有保存-->
			...
			<div class="tTable">
				<el-table :data="newTableData" stripe border style="width: 100%;" size="mini">
					...
					<el-table-column prop="total_num" label="数量" width="150" align="center">
						<template slot-scope="scope">
							<span v-if="disabled">{{scope.row.total_num}}</span>
							<el-input v-else size="small" v-model="scope.row.total_num" placeholder="" align="center" @change="multiply(scope.row)"></el-input>
						</template>
					</el-table-column>
			...
			<div class="search1">
				<el-form label-width="80px" :inline="true" :model="forMations" class="demo-form-inline">
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="摘要">
								<el-input v-model="forMations.abstract" placeholder="请输入" style="width:220px;" :disabled="disabled"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="制单人">
