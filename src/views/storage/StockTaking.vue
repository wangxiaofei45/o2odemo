	...
	<div>
		...
		<div class="tab-container" v-show="isShow == 1">
			<el-form label-width="80px" :inline="true" v-model="formInline" class="demo-form-inline">
				<div class="title">
					<el-row>
						<el-col>
							<el-button v-show="permission.indexOf('136') != -1" type="primary" @click="addPurseMan" style="margin-right: 20px;">
				...
				<el-table :data="data" stripe border style="width: 100%;" size="mini">
					...
					<el-table-column prop="document_num" label="单据号" fixed width="150" align="center">
						<template slot-scope="scope">
							<!--点击单据号跳转显示单据详情-->
							<span v-if="permission.indexOf('138') != -1" @click="goToSeeEditor(scope.row)" style="color: #18CCBA;cursor:pointer;">{{scope.row.document_num}}</span>
					...
					<el-table-column prop="number" label="盈亏数量" width="200" align="center">
					</el-table-column>
					<el-table-column prop="money" label="盈亏金额" width="150" align="center">
						<template slot-scope="scope">
					...
					<el-table-column prop="status" label="审核状态" width="100" align="center">
						<template slot-scope="scope">
							<span v-if="scope.row.status == 0" type="text">未提交</span>
							<span v-if="scope.row.status == 1" type="text">已提交</span>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" width="200" fixed="right" align="center">
						<template slot-scope="scope" v-if="scope.row.status == 0">
							<el-button v-show="permission.indexOf('139') != -1" size="mini" type="warning" @click="bePutInStorage(scope.row.id)">提交</el-button>
							<el-button v-show="permission.indexOf('235') != -1" size="mini" type="danger" @click="deletes(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
		...
		<div class="model" v-show="PutInStorageModel">
			<div class="PutInStorage">
				...
				<el-row class="bottomButton bottom">
					<el-col :span='24'>
		...
		<div class="tab-container" v-show="isShow == 2">
			<el-row class="model_title">
				<el-col :span="12" style="border-left: #13C2C2 3px solid; padding-left: 15px;">
					<span v-if="!disabled">新增</span>库存盘点单
				</el-col>
				<el-col :span="12" style="text-align: right;">
			...
			<div class="title">
				<el-row v-show="!disabled">
					<el-col :span="24">
						<!--新增时候的保存-->
						<el-button type="primary" @click="saveNotAudit" v-if="showSave == 1">
							<svg-icon icon-class="icon_save" style="margin-right: 10px;" /> <span>保存</span>
						</el-button>
						<!--新增的时候保存并提交-->
						<el-button type="primary" @click="saveandsubmit" v-if="showSave == 1">
							<svg-icon icon-class="icon_submit" style="margin-right: 10px;" /> <span>提交</span>
						</el-button>
						<!--编辑时候的保存-->
						<el-button v-show="permission.indexOf('137') != -1" type="primary" @click="EditOutOrder" v-if="showSave == 2">
							<svg-icon icon-class="icon_save" style="margin-right: 10px;" /> <span>保存</span>
						</el-button>
						<!--查看的时候没有保存-->
						<el-button type="primary" v-if="showSave == 2" @click="sureBePutInStorage">
							<svg-icon icon-class="icon_submit" style="margin-right: 10px;" /> <span>提交</span>
						</el-button>
					</el-col>
				</el-row>
			...
			<div class="search1">
				<el-form label-width="80px" :inline="true" :model="forMation" class="demo-form-inline">
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="盘点类型">
								<el-select v-model="forMation.type" placeholder="请选择" @change="selectSupplier" style="width: 200px;" :disabled="disabled">
									<el-option v-for="item in takeStore" :label="item.name" :value="item.value" :key="item.value"></el-option>
								</el-select>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="摘要">
								<el-input v-model="forMation.abstract" placeholder="请输入" style="width:220px;" :disabled="disabled"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="制单人">
			...
			<el-row class="search2" style="margin-top:20px;margin-bottom:10px;">
				...
				<el-col :span="12" style="text-align: right;" v-show="!typeDisabled">
					<el-input placeholder="请输入内容" v-model="typeAndBarCode.bar_code" class="input-with-select" style="width: 400px;" @keyup.enter.native="addgoods" :disabled="disabled">
						<el-select v-model="typeAndBarCode.type" slot="prepend" placeholder="请选择" :disabled="disabled">
			...
			<div class="tTable">
				<el-table :data="newTableData" stripe border style="width: 100%;" size="mini">
					...
					<el-table-column prop="number" label="库存数量" width="150" align="center">
					</el-table-column>
					...
					<el-table-column prop="inventory_number" label="盘点数量" width="150" align="center">
						<template slot-scope="scope">
							<span v-if="disabled">{{scope.row.inventory_number}}</span>
							<el-input v-else size="small" v-model="scope.row.inventory_number" placeholder="" align="center" @change="multiply(scope.row)"></el-input>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" width="200" fixed="right" align="center" v-if="!disabled">
						<template slot-scope="scope" v-if="!typeDisabled">
							<el-button  size="mini" type="danger" @click="deleteAdd()">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
			...
			<div class="model" v-show="open_new_rult">
				<div class="model_con">
					<el-row class="model_title">
						...
						<el-col :span="12" style="text-align: right;">
							<el-button type="text" @click="cancel_new_rult" name="topRightCancel">
								<svg-icon icon-class="cancel" />
							</el-button>
						</el-col>
					</el-row>
	...
	export default {
		data() {
			return {
				...
				currentVendor: {},
				StaticGetBillingUser:[],//获取的制单人的列表
			}
		},
		created() {
			this.ajaxjson();
		...
		methods: {
			ajaxjson() {
				...
				this.$post(this.$inventoryList, postData).then((res) => {
			...
			multiply(e) {
				let inventory_number = Number(e.inventory_number);
				let numbers = Number(e.number);
				let price = Number(e.price);
				let contrast_number = Number(e.contrast_number);
				let contrast_money = Number(e.contrast_money);
				e.contrast_number = inventory_number - numbers;
				e.contrast_money = e.contrast_number * price;
			},
			//分页
			...
			goToSeeEditor(e) {
				...
				this.$post(this.$inventoryShopDetail, postData).then((res) => {
					if(res.status_code == 0) {
						this.forMation = res.data.inventory;
				...
				if(postData.type == 1) {
					this.$post(this.$staticGetCodeGoods, postData).then((res) => {
						if(res.status_code == 0) {
							...
							datas.number = datas.reserve_count; //库存
							datas.inventory_number = 0;
							datas.contrast_number = 0;
							datas.contrast_money = 0;
							let canAdd = true;
			...
			sureAddNext() {
				...
				for(let i = 0; i < Arr.length; i++) {
					...
					Arr[i].number = Arr[i].reserve_count; //库存
					Arr[i].inventory_number = Number(Arr[i].stock); //盘点数量
					Arr[i].contrast_number = Arr[i].inventory_number - Number(Arr[i].number); //盘亏数量
					Arr[i].contrast_money = Arr[i].contrast_number * Number(Arr[i].price); //盘亏金额
					delete Arr[i].color;
					delete Arr[i].size;
					...
					if(newTableData.length == 0) {
						...
						for(let j = 0; j < newTableData.length; j++) {
							if(Arr[i].sku_id == newTableData[j].sku_id) {
								newTableData[j].inventory_number = Number(newTableData[j].inventory_number) + Number(Arr[i].inventory_number);
								newTableData[j].contrast_number = Number(newTableData[j].contrast_number) + Number(Arr[i].contrast_number);
							} else {
								centerArr.push(Arr[i]);
							}
						}
			...
			saveNotAudit() {
				...
				this.$post(this.$inventoryAddInventory, newTableData).then((res) => {
					if(res.status_code == 0) {
						this.$message({
							type: 'success',
							message: res.message,
			...
			saveandsubmit() {
				...
				this.$post(this.$inventoryAddInventory, newTableData).then((res) => {
					if(res.status_code == 0) {
						...
						this.$post(this.$inventoryBalance, postData).then((res) => {
							if(res.status_code == 0) {
								this.$message({
									type: 'success',
									message: res.message,
			...
			EditOutOrder() {
				...
				let arrData = this.$coppyArray(this.newTableData);
				newTableData.shopArr = JSON.stringify(arrData);
				this.$post(this.$inventoryEditInventory, newTableData).then((res) => {
					if(res.status_code == 0) {
						this.$message({
							type: 'success',
							message: res.message,
