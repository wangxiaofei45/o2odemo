					...
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="调入店铺">
								<el-select v-model="formInline.shop_id" placeholder="请选择供应商" style="width: 200px;">
	...
	export default {
		...
		methods: {
			ajaxjson() {
				...
				this.$post(this.$transferProcessingShop, postData).then((res) => {
					if(res.status_code == 0) {
						this.data = res.data.list.data;
						this.total = res.data.list.total; //多少条
						this.per_page = res.data.list.per_page; //当前一页多少条
					} else {
						this.$message({
							type: 'error',
							message: res.message,
			...
			getSummaries(param) {
				...
				columns.forEach((column, index) => {
					...
					if(column.property === "count_money") {
						values = data.map(item => Number(item.count_money));
					}
					if(column.property == "total_num" || column.property == "count_money") {
