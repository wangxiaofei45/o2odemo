<template>
	<div>
		...
		<div class="tab-container" v-show="isShow == 1">
			<el-form label-width="80px" :inline="true" v-model="formInline" class="demo-form-inline">
				<div class="title">
					<el-row>
						<el-col>
							<el-button v-show="permission.indexOf('146') != -1" type="primary" @click="addPurseMan" style="margin-right: 20px;">
								<svg-icon icon-class="add" style="margin-right: 10px;" /> <span>新增</span>
							</el-button>
							<el-input placeholder="输入单据号" style="width:300px" v-model="formInline.document_num">
							...
							<el-button type="primary" @click="showSearch = !showSearch" style="margin-left: 20px;">
						...
						<el-col :span="24">
							<el-form-item label="单据状态">
								<el-radio-group v-model="formInline.status">
									...
									<el-radio :label="1">已报损</el-radio>
									<el-radio :label="0">未报损</el-radio>
								</el-radio-group>
							</el-form-item>
						</el-col>
					</el-row>
			...
			<div class="tTable" id="printMe">
				<el-table :data="data" stripe border style="width: 100%;" size="mini" show-summary :summary-method="getSummaries">
					...
					<el-table-column prop="status" label="单据状态" width="100" align="center">
						<template slot-scope="scope">
							<span v-if="scope.row.status == 0">未报损</span>
							<span v-if="scope.row.status == 1">已报损</span>
						</template>
					</el-table-column>
		...
		<div class="tab-container" v-show="isShow == 2">
			<el-row class="model_title">
				<el-col :span="12" style="border-left: #13C2C2 3px solid; padding-left: 15px;">
					<span v-if="!disabled">新增</span>报损单
				</el-col>
				<el-col :span="12" style="text-align: right;">
			...
			<div class="title">
				<el-row v-show="!disabled">
					<el-col :span="24">
						...
						<!--编辑时候的保存-->
						<el-button v-show="permission.indexOf('148') != -1" type="primary" @click="EditOutOrder" v-if="showSave == 2">
							<svg-icon icon-class="icon_save" style="margin-right: 10px;" /> <span>保存</span>
						</el-button>
						<!--查看的时候没有保存-->
			...
			<div class="model" v-show="open_new_rult">
				<div class="model_con">
						...
						<el-table :data="specificationOfGoods" stripe border max-height="400" size="mini" border>
							...
							<el-table-column prop="stock" label="数量" width="150" align="center">
								<template slot-scope="scope">
									...
									<el-input v-else size="small" v-model="scope.row.stock" placeholder="请输入" align="center"></el-input>
								</template>
							</el-table-column>
					...
					<el-row>
						<el-col :span="24" style="margin-top:20px; text-align: right;" class="bottomButton">
					...
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="盘点类型">
								<el-select v-model="forMation.type" placeholder="请选择" style="width: 200px;" :disabled="disabled">
			...
			getSummaries(param) {
				...
				columns.forEach((column, index) => {
					...
					if(column.property === "number") {
						values = data.map(item => Number(item.number));
					}
					if(column.property === "money") {
						values = data.map(item => Number(item.money));
					}
					...
					if(newTableData.length == 0) {
						...
						for(let j = 0; j < newTableData.length; j++) {
							if(Arr[i].sku_id == newTableData[j].sku_id) {
								newTableData[j].total_num = Number(newTableData[j].total_num) + Number(Arr[i].total_num);
							} else {
								centerArr.push(Arr[i]);
							}
						}
