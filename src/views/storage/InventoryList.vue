		...
		<div class="tab-container">
			<el-form label-width="80px" :inline="true" v-model="formInline" class="demo-form-inline">
				<div class="title">
					<el-row>
						<el-col :span="12">
							<el-input placeholder="请输入货号" style="width:300px" v-model="formInline.document_num">
			...
			<div class="tTable" id="printMe">
				<el-table :data="data" stripe border style="width: 100%;" show-summary :summary-method="getSummaries" size="mini">
					<el-table-column type="index" label="序号" :index="indexMethod" fixed width="80" align="center">
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			upload(){
				...
				return this.$inventoryReserveExport +"?shop_id="+shop_id;
			},
			lead(){
			...
			getSummaries(param) {
				...
				columns.forEach((column, index) => {
					...
					if(column.property === "reserve_count") {
						values = data.map(item => Number(item.reserve_count));
					}
					if(column.property === "stock_count_money") {
						values = data.map(item => Number(item.stock_count_money));
					}
					if(column.property === "tag_count_money") {
						values = data.map(item => Number(item.tag_count_money));
					}
					if(column.property == "reserve_count" || column.property == "stock_count_money" || column.property == "tag_count_money") {
