	...
	<div class="step_con">
		...
		<div class="tablist" v-show="showModel == 1">
			<el-row style="margin-bottom: 15px;">
				<el-col :span="12">
					...
					<el-radio-group v-model="type" @change="changeShowType" style="margin-left: 20px;">
						<el-radio :label="1">货号</el-radio>
						<el-radio :label="2">条码</el-radio>
					</el-radio-group>
				</el-col>
				<!--功能-->
				<el-col :span="12" style="text-align: right;" v-show="type==1">
					<el-button type="primary">
						<svg-icon icon-class="icon_Import" style="margin-right: 10px;" /><span>导入</span>
					</el-button>
				</el-col>
				<el-col :span="12" style="text-align: right;" v-show="type==2">
					<a :href="returnUrl()">
						<el-button type="primary">
							<svg-icon icon-class="icon_Import" style="margin-right: 10px; transform:rotate(180deg)" /><span>导出</span>
						</el-button>
					</a>
				</el-col>
			</el-row>
			<el-table v-show="type==1" :data="tableData" stripe border style="width: 100%">
				<el-table-column type="index" label="序号" width="80" align="center">
				</el-table-column>
				<el-table-column type="index" label="商品主图" width="120" align='center'>
					<template slot-scope="scope">
						<img style="height: 30px;" :src="scope.row.image_url" />
					</template>
				</el-table-column>
				<el-table-column prop="freight_number" label="货号" min-width="150" align='center'>
					<template slot-scope="scope">
						<span style="cursor:pointer;color: #18CCBA" @click="edit(scope.row.id)">{{scope.row.freight_number}}</span>
					</template>
				</el-table-column>
				<el-table-column prop="goods_name" label="商品名称" min-width="100" align='center'>
				</el-table-column>
				<!--<el-table-column prop="min_price" label="最小价格" width="90" align='center'>
					<template slot-scope="scope">
						<span>{{scope.row.min_price/100}}</span>
					</template>
				</el-table-column>-->
				<el-table-column prop="max_price" label="价格" min-width="60" align='center'>
					<template slot-scope="scope">
						<span>{{scope.row.max_price/100}}</span>
					</template>
				</el-table-column>
				<el-table-column prop="p_category_name" label="一级分类" min-width="80" align='center'>
				</el-table-column>
				<el-table-column prop="category_name" label="二级分类" min-width="80" align='center'>
				</el-table-column>
				<el-table-column prop="brand_name" label="品牌">
				</el-table-column>
			</el-table>
			<!--条码显示-->
			<el-table v-show="type==2" :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column prop="freight_number" label="货号" min-width="100" align='center'>
					<template slot-scope="scope">
						<span style="cursor:pointer;color: #18CCBA" @click="edit(scope.row.id)">{{scope.row.freight_number}}</span>
					</template>
				</el-table-column>
				<el-table-column prop="attr_1_value" label="颜色" min-width="80" align='center'>
				</el-table-column>
				<el-table-column prop="attr_2_value" label="尺码" min-width="80" align='center'>
				</el-table-column>
				<el-table-column prop="barcode" label="条形码" min-width="150" align='center'>
				</el-table-column>
				<el-table-column prop="goods_name" label="商品名称" min-width="100" align='center'>
				</el-table-column>
				<el-table-column prop="p_category_name" label="一级分类" min-width="80" align='center'>
			...
			<el-row v-show="type==2" style='background-color: #f5f5f5;padding: 10px 25px;'>
				<el-checkbox v-model="checkedAll">全选</el-checkbox>
			</el-row>
			<div class="block" style="padding: 15px; background-color: #fff; text-align: right;">
			...
			<div class="model" v-show="openToLead">
				<div class="toLeadModel">
					...
					<el-row class="leadcon" style="padding: 50px;0px">
						<el-col :span="8">
						...
						<el-col :span="16">
							...
							<el-col :sapn="24" class='button_con'>
								<a href="/static/excel/importTemplate.xlsx" download="importTemplate.xlsx">
									<el-button type="primary">
										下载模板
									</el-button>
								</a>
			...
			<div class="model" v-show="UploadExcel">
				<div class="UploadModel">
					...
					<el-row>
						<el-upload ref="upload" class="upload" :action="uploadUrl()" :on-success="handleSuccess" :limit=1 multiple method:="post" :show-file-list="false" :on-preview="handlePreview" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel,.csv,text/plain" :file-list="fileList">
			...
			<div class="model" v-show="finish">
				<div class="UploadModel">
					...
					<el-row>
						<el-col :span="24" v-for="(item,index) in finishMessage" :key="index" style="padding: 10px;">
							{{item}}
						</el-col>
						<el-col :span="24">
					...
					<el-row class="step2_info">
						<el-row class="step2_info_1">
							...
							<el-form-item label="商品分类" prop="good_class">
								<el-input v-model="amendInfo.categoryName" style="max-width: 900px;" disabled></el-input>
							</el-form-item>
							<el-form-item label="商品名称" prop="goods_name">
								<el-input v-model="amendInfo.goods_name" style="max-width: 900px;" placeholder="请输入" disabled></el-input>
								<div class="hint">商品标题名称长度至少3个字符,最长50个汉字</div>
							</el-form-item>
							<el-form-item label="货号" prop="freight_number">
								<el-input v-model="amendInfo.freight_number" style="max-width: 900px;" placeholder="请输入" maxlength="8" disabled></el-input>
							...
							<el-row class="type">
								<el-col :span="12" v-for="(item,index) in categoryAttributes" :key="item.id">
									<el-form-item :label="item.name">
										<el-select v-model="item.modelName" placeholder="请选择" style="width: 300px;">
							...
							<el-row>
								<el-form-item label="颜色分类">
									...
									<div class="selectColor" v-for="(item,index) in amendCenterColor.data">
										<el-input v-model="item.name" style="width: 300px;" placeholder="选择或输入主色" disabled></el-input>
										<el-input style="width: 300px; height: 36px;" v-model="item.colorRemark" placeholder="备注(如偏深偏浅等)" disabled></el-input>
								...
								<el-form-item style="margin-top: 15px;" label="商品库存" prop="stock">
									<el-input v-model="amendInfo.stock" style="width: 300px;" disabled></el-input>
								</el-form-item>
							</el-row>
						</el-row>
							...
							<el-form-item label="商品图片">
								...
								<div v-for="(item,index) in amendShowImg" :key="index" style="float: left;margin-right: 15px;">
									<el-upload class="avatar-uploader" :multiple="true" action="http://up.qiniu.com/" accept="image/jpeg,image/gif,image/png,image/bmp" :data="postData" :show-file-list="false" :on-success="handleAvatarSuccess_2" :before-upload="beforeAvatarUpload">
										<img v-if="amendShowImg.length" :src="domain+item.image_url" class="avatar" @click="amendupList(index)">
										<div v-else class="upload_img" @click="amendupList(index)">
		...
		<div v-show="showModel == 2">
			<el-row class="model_title">
				...
				<el-col :span="12" style="text-align: right;">
					<el-button type="text" @click="back('fromItem')">
						<svg-icon icon-class="cancel" />
					</el-button>
				</el-col>
			</el-row>
			<el-steps :active="active" align-center style="width: 800px;margin: 0 auto;">
				<el-step title="选择分类" description="">
				</el-step>
				<el-step title="商品详情" description="">
				</el-step>
			</el-steps>
			<!--商品添加第一步-->
					...
					<el-form :model="fromItem" ref="fromItem" :rules="rulefromItem" label-width="80px">
						...
						<el-row class="step2_info">
							<el-row class="step2_info_1">
								...
								<el-form-item label="商品分类" prop="good_class">
									<el-input v-model="show_value" style="max-width: 900px; margin-right: 10px;" disabled></el-input>
									<el-button size="small" type="primary" style="width: 120px;" @click="selectShowValue">切换类目</el-button>
								</el-form-item>
								<el-form-item label="商品名称" prop="goods_name">
									<el-input v-model="fromItem.goods_name" style="max-width: 900px;" placeholder="请输入"></el-input>
									<div class="hint">商品标题名称长度至少3个字符,最长50个汉字</div>
								</el-form-item>
								<el-form-item label="货号" prop="freight_number">
									<el-input v-model="fromItem.freight_number" style="max-width: 900px;" placeholder="请输入" maxlength="8"></el-input>
									<div class="hint">货号最长8位</div>
								</el-form-item>
								<el-form-item label="类目属性">
									<div class="hint">商品标题名称长度至少3个字符,最长50个汉字</div>
								...
								<el-row>
									<el-form-item label="颜色分类">
										...
										<div class="selectColor" v-for="(item,index) in centerColor">
											<el-input @focus="showColor(item,index)" @blur="closeColor(item,index)" v-model="item.colorName" style="width: 300px;margin-right: 10px;" placeholder="选择或输入主色"></el-input>
											<el-input style="width: 300px; height: 36px; margin-right: 10px;" v-model="item.colorRemark" placeholder="备注(如偏深偏浅等)"></el-input>
											<img v-if="item.showImg" style="width: 36px;height: 36px;position: absolute;top: auto; margin-left: 15px;" :src="item.showImg" />
											<el-upload v-else style="display: inline-block" :multiple="true" action="http://up.qiniu.com/" accept="image/jpeg,image/gif,image/png,image/bmp" :data="postData" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
												<el-button size="small" type="primary" style="width: 120px;" @click="upLoadListImg(item,index)">上传图片</el-button>
											</el-upload>
											<!--定位元素-->
									...
									<el-form-item label="尺码">
										...
										<div class="size">
											<el-radio-group v-model="size">
												<el-radio v-for="(item,index) in sizeArr" :key="item.id" :label="item.id" @change="checkSize">{{item.name}}</el-radio>
											</el-radio-group>
										</div>
										<div class="diySize">
											<el-input placeholder="请输入自定义项" v-model="customSize" style="width: 300px;margin-right: 10px;"></el-input>
									...
									<el-table :data="fromTable" max-height="600" border style="width: 760px;" v-show="fromTable.length">
										...
										<el-table-column prop="stock" label="库存" width="200" align="center">
											<template slot-scope="scope">
												<el-input v-model="scope.row.stock" @blur="inputBlur"></el-input>
											</template>
										</el-table-column>
										<el-table-column prop="shop_code" label="商家编码" width="200" align="center">
											<template slot-scope="scope">
									...
									<el-form-item style="margin-top: 15px;" label="商品库存" prop="stock">
										<el-input v-model="fromItem.stock" style="width: 300px;" disabled></el-input>
									</el-form-item>
								</el-row>
							</el-row>
							<!--第三部分-->
							...
							<el-row class="submit">
								<el-col :sapn="24" style="text-align: center;">
									<el-button type="primary" @click="submit('fromItem')" style='width: 33%;'>
										提交
									</el-button>
								</el-col>
							</el-row>
	...
	export default {
		data() {
			return {
				...
				tablehight: document.documentElement.clientHeight - 260, //表格高度
				tableData: [],
				total: 0,
				page: 1, //分页
				per_page: 10, //每页几条
		...
		methods: {
			...
			returnUrl() {
				...
				let educeArr = this.$coppyArray(this.educeArr);
				let sku_ids = educeArr.join(",");
				let shop_id = sessionStorage.getItem("shop_id");
				let type = this.checkedAll ? '1' : '2';
				return this.$goodsExport + "?type=" + type + "&sku_ids=" + sku_ids + "&shop_id=" + shop_id;
			},
			//打开导入
			ToLead() {
			...
			handleSuccess(res, file) {
				if(res.status_code == 0) {
					this.finishMessage = res.data.errors;
			...
			handleSelectionChange(val) {
				let that = this; //this的指向性的问题
				val.map(function(value, index) {
					that.educeArr.push(value.gsku_id);
				});
			},
			//以上的都是新增
			selectShowValue() {
			...
			inputBlur() {
				...
				for(let i = 0; i < fromTable.length; i++) {
					stock += Number(fromTable[i].stock);
				};
				this.fromItem.stock = stock;
			},
			//打开添加
			addGoods() {
				this.showModel = 2;
			...
			back(e) {
				this.showModel = 1;
				this.$refs[e].resetFields(); //重置表单
				for(let i = 0; i < this.imageUrl_1.length; i++) {
					this.imageUrl_1[i].showImg = '';
				};
				this.fromItem.imgData = ['', '', '', '', ''];
				this.size = 0;
				this.centerColor = [];
				this.fromTable = [];
				this.active = 0;
			...
			edit(e) {
				...
				this.$post(this.$goodsEdit, postData).then((res) => {
					if(res.status_code == 0) {
						...
						if(res.data.spuImages.length == 0) {
							this.amendShowImg = [{
								default: 1,
								image_url: '',
							}, {
			...
			checkSize() {
				...
				for(let i = 0; i < this.sizeArr.length; i++) {
					for(let j = 0; j < this.sizeArr[i].child.length; j++) {
						this.sizeArr[i].child[j].checked = false;
					}
				}
			},
			//自定义添加尺码
			customAdd() {
				...
				let obj = {
					...
					id: 0,
					attr_id: this.sizeArr[0].attr_id, //上级id
					attr_name: this.sizeArr[0].attr_name,
				};
			...
			closeColor(item, index) {
				...
				if(item.colorName != '') {
					if(this.checking(item, index)) {
						if(!item.color_attr_id){
							item.color_attr_id = this.salesProperty[0].id
							item.color_attr_name = this.salesProperty[0].name
						}
						//如果不是在最后一行添加的就不添加
...
//						if(!colorArr[i].color_attr_id){
//							colorArr[i].color_attr_id = this.colorArrs.id;
//							colorArr[i].color_attr_name = this.colorArrs.id;
//						}
						let Xindex = {};
			...
			handleAvatarSuccess_2(res, file) {
				this.amendShowImg[this.amendImgNumber].image_url = res.key;
				this.amendImgNumber = -1;
			},
			//提交
			submit(e) {
				...
				let sortArr = [];
				for(let i = 0; i < fromTable.length; i++) {
					sortArr.push(fromTable[i].sku_price);
				...
				let min_price = sortArr[0];
				let max_price = sortArr[sortArr.length - 1];
				postData.min_price = min_price * 100;
				postData.max_price = max_price * 100;
			...
			amend() {
				...
				for(let i = 0; i < imgData.length; i++) {
					if(imgData[i].default == 1) {
						obj3.default = imgData[1].image_url;
					} else {
...
<style scoped="scoped">
	...
	.contnet_1_title {
		...
		font-size: 14px;
		color: #18CCBA;
		background-color: rgba(24, 204, 186, 0.12);
		border: 4px;
		padding: 10px;
	}
	/*选择包含的内容*/
