		...
		<div class="title">
			<el-row>
				<el-col :span="12">
					...
					<el-select v-model="value_1" filterable placeholder="请选择" @change="select_1">
						<el-option v-for="item in options_1" :key="item.value" :label="item.label" :value="item.value">
						</el-option>
					</el-select>
					<el-select v-model="value_1[0]" filterable placeholder="请选择" @change="select_1">
						<el-option v-for="item in options_1" :key="item.value" :label="item.label" :value="item.value">
						</el-option>
					</el-select>
				</el-col>
	...
	export default {
		...
		methods: {
			...
			ajaxjson() {
				...
				//调用post请求方法
				this.$post(this.$BrandList, postData).then((res) => {
					let data = res;
					console.log(data);
			...
			submitForm(e) {
				this.$refs[e].validate((valid) => {
					if(valid) {
						...
						this.$post(this.$BrandAdd, postData).then((res) => {
							let data = res;
							if(data.status_code == 0) {
								this.$message({
									type: 'success',
			...
			amend(e) {
				...
				this.$post(this.$BrandInfo, postData).then((res) => {
					let data = res;
					if(data.status_code == 0) {
