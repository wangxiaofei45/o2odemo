	...
	<div>
		<div class="tab-container">
			...
			<div class="title">
				<el-row>
					<el-col :span="24">
						<el-button v-show="permission.indexOf('159') != -1" type="primary" @click="closeAnAbccount" style="margin-right: 20px;">
					...
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="费用名称">
								<el-input style="width:200px" placeholder="请输入" v-model="formInline.cost_name"></el-input>
							</el-form-item>
						</el-col>
			...
			<div class="tTable">
				<el-table :data="data" stripe border style="width: 100%;" size="mini">
					...
					<el-table-column prop="cost_name" label="费用名称" width="120" align="center">
					</el-table-column>
					<el-table-column prop="supplier" label="关联单号" width="100" align="center">
					</el-table-column>
					<el-table-column prop="cost_money" label="金额" width="100" align="center">
						<template slot-scope="scope">
							<span>{{scope.row.cost_money/100}}</span>
						</template>
					</el-table-column>
					...
					<el-row>
						...
						<el-col :span="12">
							<el-form-item label="费用名称" prop="cost_name">
								<el-input placeholder="请输入" v-model="accountFrom.cost_name"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="12">
			...
			<div class="model_con">
				...
				<el-form label-width="90px" :model="amendInfo" :rules="ruleaccountFrom" ref="amendInfo" class="demo-form-inline">
					<el-row>
						...
						<el-col :span="12">
							<el-form-item label="日期">
								<el-date-picker type="dateTime" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" v-model="amendInfo.storage_date">
								</el-date-picker>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item label="费用名称" prop="cost_name">
								<el-input placeholder="" v-model="amendInfo.cost_name"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="12">
	...
	export default {
		...
		methods: {
			ajaxjson() {
				...
				this.$post(this.$otherCostList, postData).then((res) => {
			...
			AddOtherCost(e) {
				this.$refs[e].validate((valid) => {
					if(valid) {
						...
						postData.cost_money = Number(postData.cost_money)*100 +'';
						this.$post(this.$otherCostAddDisburseCost, postData).then((res) => {
							let data = res;
			...
			amend(e){
				this.$refs[e].validate((valid) => {
					if(valid) {
						...
						this.$post(this.$otherCostEditDisburse, postData).then((res) => {
							let data = res;
							if(data.status_code == 0) {
								this.$message({
									type: 'success',
