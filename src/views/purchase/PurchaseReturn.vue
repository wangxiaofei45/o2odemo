	...
	<div>
		...
		<div class="tab-container" v-show="isShow == 1">
			<div class="title">
				<el-row>
					<el-col>
						<el-button v-show="permission.indexOf('226') != -1" type="primary" @click="purchasereturnAdd" style="margin-right: 20px;">
							<svg-icon icon-class="add" style="margin-right: 10px;" /> <span>新增</span>
						</el-button>
						<el-input v-show="permission.indexOf('231') != -1" placeholder="载入入库单" style="width:240px;margin-right: 20px;" v-model="Loadthwarehousereceipt" @keyup.enter.native="Loadwarehousereceipt"></el-input>
						...
						<el-button type="primary" @click="searchModel = !searchModel">
			...
			<div class="tTable">
				<el-table :data="data" stripe border style="width: 100%;" show-summary :summary-method="getSummaries" size="mini">
					...
					<el-table-column prop="is_pass" label="单据状态" width="100" align="center">
						<template slot-scope="scope">
							<span v-if="scope.row.is_pass == 0" type="text">未退货</span>
							<span v-if="scope.row.is_pass == 1" type="text">已退货</span>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" width="200" fixed="right" align="center">
						<template slot-scope="scope" v-if="scope.row.is_pass == 0">
							<el-button v-show="permission.indexOf('229') != -1" size="mini" type="warning" @click="returnedGoods(scope.row.id)">退货</el-button>
							<el-button v-show="permission.indexOf('230') != -1" size="mini" type="danger" @click="handleDelete(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
		...
		<div class="tab-container" v-show="isShow == 2">
			...
			<div class="title">
				<el-row>
					<el-col :span="24">
						<el-button v-show="permission.indexOf('226') != -1" type="primary" @click="saveNotAudit">
							<svg-icon icon-class="icon_save" style="margin-right: 10px;" /> <span>保存</span>
						</el-button>
						<el-button v-show="permission.indexOf('229') != -1" type="primary" @click="openSave">
							<svg-icon icon-class="goodsReturn" style="margin-right: 10px;" /> <span>退货</span>
						</el-button>
					</el-col>
				</el-row>
			</div>
					...
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="日期">
								<el-date-picker v-model="forMation.created_at" type="date" disabled placeholder="选择日期">
								</el-date-picker>
							</el-form-item>
						</el-col>
						<el-col :span="6">
						...
						<el-col :span="6">
							<el-form-item label="供应商">
								<el-select v-model="forMation.cbid" placeholder="请选择" @change="selectSupplier" :disabled="canChangeselectSupplier" style="width: 220px;">
									<el-option v-for="(item,index) in supplierCommonSupplierList" :label="item.name" :value="item.id" :key="item.id"></el-option>
								</el-select>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="摘要">
								<el-input v-model="forMation.abstract" placeholder="请输入" style="width:220px;"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="制单人">
			...
			<div class="tTable">
				<el-table :data="newTableData" stripe border style="width: 100%;" size="mini">
					...
					<el-table-column prop="stock" label="数量" width="100" align="center">
						<template slot-scope="scope">
							<el-input size="small" v-model="scope.row.stock" placeholder="" align="center" @change="multiply($event,scope.row,scope.row.index)"></el-input>
						</template>
					</el-table-column>
					<el-table-column prop="price" label="吊牌价" width="100" align="center">
					</el-table-column>
		...
		<div class="model" v-show="fromAndSave">
			<div class="save_model_con">
				<el-row class="model_title">
					...
					<el-col :span="12" style="text-align: right;">
						<el-button type="text" @click="cancelSave" style="padding: 0px;">
							<svg-icon icon-class="cancel" />
						</el-button>
					</el-col>
				</el-row>
		...
		methods: {
			...
			handleSelectionChange(val) {
		   	...
		   	determineLoad(){
		   		...
		   		this.canChangeselectSupplier = true;
		   		let postData = {
			...
			purchasereturnAdd() {
				...
				this.$post(this.$purchasereturnAdd,postData).then((res) => {
					if(res.status_code == 0) {
						...
						this.isShow = 2;
						this.purchasereturnInfo(this.prid);
						this.canChangeselectSupplier = false;
					} else {
						this.$message({
							type: 'error',
							message: res.message,
			...
			getSummaries(param) {
				...
				columns.forEach((column, index) => {
					...
					if(column.property == "total_price" || column.property == "stock") {
